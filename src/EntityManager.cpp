...
void EntityManager::removeDeadEntities(EntityVec &vec) {
  ...
  // probably not the fastest solution, but it is safe
    ...
    {
        vec.erase(std::remove(vec.begin(), vec.end(), deadEntity), vec.end());
    }
}

std::shared_ptr<Entity> EntityManager::addEntity(std::string entityTag) {
  // creat the entity shared pointer
  auto entity = std::shared_ptr<Entity>(new Entity(m_totalEntities++, entityTag));
...
EntityVec &EntityManager::getEntities(std::string type) {
